 ...
 * yqfang,qianzhixiang
 * Version: 1.0.0 - 2016-07-08T02:52:45.815Z
 * License: ISC
 */
...
$templateCache.put('input-checkbox.html','<div ng-if=vm.field.inline><label class=checkbox-inline ng-repeat="item in vm.field.lists"><input type=checkbox ng-click=vm.check(item) ng-checked=item.checked>{{item.desc}}</label></div><div ng-if=!vm.field.inline><div class=checkbox ng-repeat="item in vm.field.lists"><label><input type=checkbox ng-click=vm.check(item) ng-checked=item.checked> {{item.desc}}</label></div></div>');
...
$templateCache.put('input-text.html','<input type=text id={{componentCtrl.field.name}} name={{componentCtrl.field.name}} ng-model=componentCtrl.ref.model ng-required=componentCtrl.field.required required-message="\'{{componentCtrl.field.requiredMsg}}\'" ng-maxlength={{componentCtrl.field.maxlength}} ng-minlength={{componentCtrl.field.minlength}} ng-pattern=componentCtrl.field.pattern validate-on={{componentCtrl.field.validateOn}} validator={{componentCtrl.field.validator}} invalid-message={{componentCtrl.field.validator}} class=form-control up-max-length={{componentCtrl.field.maxwrite}} up-disabled={{componentCtrl.field.disabled}} up-http-bind={{componentCtrl.field.bind}} ng-attr-placeholder={{componentCtrl.field.placeholder}} ng-style=componentCtrl.field.style>');
...
uf.directive('mayaCheckbox', ["$state", "$timeout", "dialogs", function($state, $timeout, dialogs) {
    return {
        restrict: 'EA',
...
uf.directive("upDisabled", ["$parse", function ($parse) {
    return {
        require: "?^uForm",
        restrict: 'A',
...
uf.directive("upHttpBind", ["$parse", "$http", function ($parse,$http) {
